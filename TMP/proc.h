...
struct	pentry	{
	...
	int	pwaitret;
	int locks[NLOCKS];	//locks acquired by the process: 0 no lock; 1 reader; 2 writer
	int waittime; 		//waiting time on a lock queue
	int waittype;		//type of process waiting: 0 not waiting; 1 read; 2 write
	int waitqueue;		//the lock queue on which the process is waiting
	int actprio;		//the original priority of the process
	int ldeleted;		//lock deleted
};
